# Subrutina para estimacion de los efectos aleatorios

EffectsEst.BBNR <- function(y,m,beta,u,p,phi,D.,X,Z,maxiter){
  
  #OJO: El argumento p no es utilizado, se sobreescribe durante la rutina
  # Comentarios en ingles son del codigo original

    # 1. Inicializacion de valores
  ...
  while (max(abs((eta-oldeta)/eta))>0.001){
    ...
    while (max(abs((beta-oldbeta)/beta))>0.001){
      ...
      oldbeta <- beta
      p <- 1/(1+exp(-(X%*%beta+Z%*%u))) # Este es el unico punto donde entran los RE
      
      # Aparentemente genera matriz de varianza estimada por observacion
      # Compute S
      s <- p*(1-p)
      S <- diag(c(s))
      
      # No se que son t, v pero son 0 cuando y = 0 o m (maximo de la binomial)
      # Compute t and v
      t <- NULL
      v <- NULL
      for (j in 1:nObs){
        ...
        if (y[j]==m.[j]){ # Codigo original usa m en vez de m. pero eso da error
          t2 <- 0
          v2 <- 0
        }else{
